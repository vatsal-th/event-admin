...
import { X, Loader2, Globe, CheckCircle2 } from 'lucide-react';
import { addCountry, updateCountry } from '../store/slices/countrySlice';
...
const CountryModal = ({ country, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (country) {
                await dispatch(updateCountry({ id: country._id, data: formData })).unwrap();
            } else {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden animate-in fade-in zoom-in duration-300">
                {/* Header */}
                <div className="px-6 py-5 bg-gradient-to-r from-blue-600 to-purple-600 flex items-center justify-between">
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="flex gap-4 mt-8">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="flex-1 px-6 py-3 text-sm font-bold text-white bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl hover:opacity-90 transition-all shadow-lg shadow-blue-200 disabled:opacity-50 disabled:shadow-none flex items-center justify-center gap-2 cursor-pointer"
                        >
                            {loading ? (
                                <>
                                    <Loader2 size={18} className="animate-spin" />
                                    {country ? 'Updating...' : 'Adding...'}
                                </>
                            ) : (
